			...
			if (source.equals(target)) {
				...
				try {
					...
					if (!source.equals("BRL")) {
							...
							if (resp.getValue().isEmpty()) throw new Exception("Moeda não encontrada para" + source);
					...
					if (!target.equals("BRL")) {
						...
						if (resp.getValue().isEmpty()) throw new Exception("Moeda não encontrada para" + target);
